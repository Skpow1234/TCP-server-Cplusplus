...
[[nodiscard]] auto any_pending_writes(const std::vector<ConnSlot>& slots) -> bool {
    for (const auto& s : slots) {
        if (!s.conn.write_buffer().empty()) {
            return true;
        }
    }
    return false;
...
void run_server(
    ...
    tcp_server::core::ShutdownCoordinator& lifecycle,
    std::atomic<int>& completions) {
    ...
    while (lifecycle.should_poll_network() && clock::now() < deadline) {
        if (!lifecycle.should_accept_new_connections() && !listener_unregistered) {
        ...
        for (std::size_t ei = 0; ei < count; ++ei) {
            ...
            if (ev.socket == acceptor.listener_handle()) {
                if (!lifecycle.should_accept_new_connections()) {
                    continue;
                }
        ...
        lifecycle.try_advance_to_stopped_if_drained(
            listener_unregistered,
            slots.size(),
            any_pending_writes(slots));
    }

    assert(lifecycle.phase() == tcp_server::core::ShutdownPhase::Stopped);

    while (!slots.empty()) {
        (void)poller.erase(slots.back().conn.native_handle());
        slots.pop_back();
    }
    if (!listener_unregistered) {
        assert(acceptor.unregister_listener(poller).has_value());
    }
}
...
void client_session(std::uint16_t port, int id, std::atomic<int>& connected) {
#if defined(_WIN32)
    ...
    const std::string payload = "shutdown client " + std::to_string(id);
...
int main() {
    ...
    std::thread server_thread([&] { run_server(server_port, listening, lifecycle, completions); });
    ...
    for (int i = 0; i < k_num_clients; ++i) {
        clients.emplace_back([port, i, &connected] { client_session(port, i, connected); });
    }

    while (connected.load(std::memory_order_acquire) < k_num_clients) {
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
    }
